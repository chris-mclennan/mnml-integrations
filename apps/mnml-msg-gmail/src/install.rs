...
//! manifest at `~/.config/mnml/integrations/gmail.toml` so mnml
...
const INTEGRATION_ID: &str = "gmail";
...
pub fn install() -> Result<()> {
    let spec = IntegrationSpec {
        ...
        chip: Some(ChipSpec {
            glyph: "\u{F03BC}".into(), // nf-md-gmail
            fallback: "Gm".into(),
            color: "red".into(),
            enabled: true,
