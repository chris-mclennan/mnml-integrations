...
pub fn interactive_login(creds: &ClientCreds) -> Result<Token> {
    ...
    // Random `state` token — the OAuth defense against local-loopback
    // hijack. Without it, any process that connects to 127.0.0.1:<port>
    // before the browser does wins the auth code (a Chrome extension,
    // a security scanner, even a curious sibling process running an
    // accept loop). Google echoes `state` back in the redirect URL;
    // we keep accepting until we see one matching ours and drop
    // everything else. sibling-crates-hunt-2026-06-08 #3 HIGH-severity
    // finding.
    let state = generate_state_token();
    let auth_url = build_auth_url(&creds.client_id, &redirect_uri, &state);
    ...
    // Accept loop with a wall-clock deadline. Each connection that
    // doesn't carry our `state` value gets dropped (with a 4xx
    // response so the wrong-callback caller sees a clear failure).
    // The browser eventually arrives with the right state and we
    // proceed.
    ...
    let code = loop {
        let remaining = deadline
            ...
            .ok_or_else(|| {
                anyhow!(
                    "timed out waiting for OAuth redirect ({}s) — did the browser callback?",
                    OVERALL_TIMEOUT.as_secs()
                )
            })?;
        listener
            .set_nonblocking(true)
            .context("set listener nonblocking for poll")?;
        // Poll with a short tick so we honor the deadline and Ctrl+C
        // remains responsive.
            ...
            match listener.accept() {
                ...
                Err(e) if e.kind() == std::io::ErrorKind::WouldBlock => {
                    if std::time::Instant::now() >= deadline {
                        return Err(anyhow!(
                            "timed out waiting for OAuth redirect ({}s)",
                            OVERALL_TIMEOUT.as_secs()
                        ));
                    }
                    std::thread::sleep(Duration::from_millis(100));
                }
                Err(e) => return Err(e).context("accept redirect"),
            }
            // Don't loop forever inside this inner accept poll — every
            // 250 ms wake back up to re-check the outer remaining budget.
            if remaining < Duration::from_millis(250) {
                break listener.accept().context("accept redirect")?;
            }
        };
        stream
        ...
        match parse_redirect_path(&path) {
            Ok((got_code, got_state)) if got_state == state => {
                let _ = write_redirect_response(&mut stream);
                break got_code;
            }
            Ok((_, got_state)) => {
                // State mismatch — almost certainly a stray local
                // connection. Tell the caller, drop it, and keep
                // listening. Don't log the actual values — they're
                // sensitive.
                eprintln!(
                    "loopback: ignored redirect with mismatched state ({} bytes)",
                    got_state.len()
                );
                let _ = write_state_mismatch_response(&mut stream);
                continue;
            }
            Err(e) => {
                eprintln!("loopback: ignored malformed redirect: {e}");
                let _ = write_state_mismatch_response(&mut stream);
                continue;
            }
        }
...
/// Generate a 128-bit random state token, hex-encoded. Reads from
/// `/dev/urandom` (macOS / Linux) — gmail is platform-scoped to
/// those targets via `webbrowser` already, and bringing in the
/// `getrandom` crate as a dependency for one read isn't worth it.
/// On a CSPRNG-read failure we fall back to a SystemTime + PID mix,
/// which is weaker but still better than no state at all (the
/// realistic attacker is a local process racing the browser, not
/// a network adversary).
fn generate_state_token() -> String {
    ...
    let mix = nanos ^ pid.wrapping_shl(64) ^ (pid as u128);
    hex_encode(&mix.to_le_bytes())
}
...
#[allow(dead_code)] // superseded by parse_redirect_path; kept for API stability
fn parse_code_from_path(path: &str) -> Result<String> {
    parse_redirect_path(path).map(|(c, _s)| c)
}

/// Parse the redirect URL's query string into `(code, state)`.
/// `state` is the empty string when omitted, so a callback without
/// state still fails the equality check against the expected token
/// in `interactive_login`'s accept loop — i.e., it's safely rejected
/// rather than spuriously accepted.
fn parse_redirect_path(path: &str) -> Result<(String, String)> {
    // path looks like `/?code=4/0Adeu5BV…&state=…&scope=…`
    let q = path
        .find('?')
    ...
    let code = code.ok_or_else(|| anyhow!("oauth redirect had no `code` param: {path:?}"))?;
    Ok((code, state))
}

/// HTTP response for a connection that failed the `state` check —
/// neither the browser callback nor any other valid path we expect
/// to handle. Tells the connecting party (often a Chrome extension
/// or a probe) why we ignored them, without leaking which `state`
/// value would have been valid.
fn write_state_mismatch_response(stream: &mut TcpStream) -> Result<()> {
    let body = "<!doctype html><title>mnml-msg-gmail</title>\
                <p>Wrong state — this connection didn't come from the OAuth flow tmnl just opened. \
                Ignored.</p>";
    let payload = format!(
    ...
    stream
        ...
        .context("write state-mismatch response")?;
    Ok(())
}
...
mod tests {
    ...
    #[test]
    fn parse_redirect_path_extracts_code_and_state() {
        let (code, state) = parse_redirect_path("/?code=ABC&state=deadbeef&scope=x").unwrap();
        assert_eq!(code, "ABC");
        assert_eq!(state, "deadbeef");
    }

    #[test]
    fn parse_redirect_path_returns_empty_state_when_missing() {
        let (code, state) = parse_redirect_path("/?code=ABC").unwrap();
        assert_eq!(code, "ABC");
        assert_eq!(state, "");
    }

    #[test]
    fn parse_redirect_path_surfaces_error_param() {
        let err = parse_redirect_path("/?error=access_denied&state=x").unwrap_err();
        assert!(err.to_string().contains("access_denied"));
    }

    #[test]
    fn generate_state_token_is_nonempty_hex_and_varies() {
        let s = generate_state_token();
        assert!(!s.is_empty());
        assert!(s.chars().all(|c| c.is_ascii_hexdigit()));
        let s2 = generate_state_token();
        assert_ne!(s, s2, "state tokens should not collide");
    }
