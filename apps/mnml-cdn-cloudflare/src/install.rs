...
//! manifest at `~/.config/mnml/integrations/cloudflare.toml`
...
const INTEGRATION_ID: &str = "cloudflare";
...
pub fn install() -> Result<()> {
    let spec = IntegrationSpec {
        ...
        commands: vec![CommandSpec {
            id: "cloudflare.open".into(),
            title: "Cloudflare: open".into(),
            group: Some("integrations".into()),
            keys: vec!["<leader>iF".into()],
            run: ":term mnml-cdn-cloudflare".into(),
        }],
        ..Default::default()
    };
