...
//! manifest at `~/.config/mnml/integrations/gcal.toml` so mnml
...
pub fn install() -> Result<()> {
    let spec = IntegrationSpec {
        ...
        chip: Some(ChipSpec {
            glyph: "\u{F0EDE}".into(), // nf-md-calendar_month
            fallback: "Ca".into(),
            color: "blue".into(),
            enabled: true,
        ...
        commands: vec![CommandSpec {
            id: "gcal.open".into(),
            title: "Google Calendar: open".into(),
            group: Some("integrations".into()),
            keys: vec!["<leader>iC".into()],
            run: ":term mnml-msg-gcal".into(),
        }],
        ..Default::default()
    };
