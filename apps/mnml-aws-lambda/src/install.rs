...
//! manifest at `~/.config/mnml/integrations/lambda.toml` so mnml
//! picks up the rail chip + palette command + chord binding on
//! next startup.
//!
//! 2026-08-01 — Stage 2 of the mnml-bridge 0.4 sibling-icons SDK.
//! Instead of relying on mnml core baking `assets/glyphs/aws/lambda.svg`
//! into MnmlSymbols.ttf at a codepoint mnml chose, we now ship our
//! own SVG in-repo (`assets/icons/lambda.svg`) and declare it via
//! `ChipSpec::glyph_svg`. `install_integration` copies the SVG to
//! `~/.config/mnml/glyphs/lambda.svg`; mnml discovers it on next
...
//!
//! `glyph_codepoint = "F1B01"` pins the SVG at the same codepoint
//! mnml core used to bake it at (see `src/glyph_builder.rs`'s
//! `BUILTIN_GLYPHS` in mnml pre-Stage-2), so users who already have
//! MnmlSymbols.ttf on their system don't see the lambda chip change
...
const INTEGRATION_ID: &str = "lambda";

fn lambda_svg_path() -> Option<PathBuf> {
    if let Ok(exe) = std::env::current_exe()
        && let Some(dir) = exe.parent()
    {
        let cand = dir.join("assets/icons/lambda.svg");
        if cand.exists() {
            return Some(cand);
        }
        let mut cur = dir.to_path_buf();
        while cur.pop() {
            let cand = cur.join("assets/icons/lambda.svg");
            if cand.exists() {
                return Some(cand);
            }
        }
    ...
    let cand = cwd.join("assets/icons/lambda.svg");
    if cand.exists() {
        return Some(cand);
    }
    None
...
pub fn install() -> Result<()> {
    let spec = IntegrationSpec {
        ...
        chip: Some(ChipSpec {
            ...
            glyph_svg: lambda_svg_path(),
            glyph_codepoint: Some("F1B01".into()),
        }),
        commands: vec![CommandSpec {
            id: "lambda.open".into(),
            title: "Lambda: open".into(),
            group: Some("integrations".into()),
            keys: vec!["<leader>iL".into()],
            run: ":term mnml-aws-lambda".into(),
        }],
        ..Default::default()
    };
