...
const INTEGRATION_ID: &str = "ecs";

fn ecs_svg_path() -> Option<PathBuf> {
    if let Ok(exe) = std::env::current_exe()
        && let Some(dir) = exe.parent()
    {
        let cand = dir.join("assets/icons/ecs.svg");
        if cand.exists() {
            return Some(cand);
        }
        let mut cur = dir.to_path_buf();
        while cur.pop() {
            let cand = cur.join("assets/icons/ecs.svg");
            if cand.exists() {
                return Some(cand);
            }
        }
    ...
    let cand = cwd.join("assets/icons/ecs.svg");
    if cand.exists() {
        return Some(cand);
    }
    None
...
pub fn install() -> Result<()> {
    let spec = IntegrationSpec {
        ...
        commands: vec![CommandSpec {
            ...
            keys: vec!["<leader>ie".into()],
            run: ":term mnml-aws-ecs".into(),
        }],
        ..Default::default()
    };
