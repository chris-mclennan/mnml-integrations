...
const INTEGRATION_ID: &str = "sqs";

fn sqs_svg_path() -> Option<PathBuf> {
    if let Ok(exe) = std::env::current_exe()
        && let Some(dir) = exe.parent()
    {
        let cand = dir.join("assets/icons/sqs.svg");
        if cand.exists() {
            return Some(cand);
        }
        let mut cur = dir.to_path_buf();
        while cur.pop() {
            let cand = cur.join("assets/icons/sqs.svg");
            if cand.exists() {
                return Some(cand);
            }
        }
    ...
    let cand = cwd.join("assets/icons/sqs.svg");
    if cand.exists() {
        return Some(cand);
    }
    None
...
pub fn install() -> Result<()> {
    let spec = IntegrationSpec {
        ...
        commands: vec![CommandSpec {
            ...
            keys: vec!["<leader>iq".into()],
            run: ":term mnml-aws-sqs".into(),
        }],
        ..Default::default()
    };
