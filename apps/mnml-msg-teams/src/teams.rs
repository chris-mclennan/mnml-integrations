        ...
        if status == StatusCode::UNAUTHORIZED {
                ...
                && let Ok(fresh) = refresh_token(&rt)
            {
                ...
                *self.token.write().unwrap() = fresh;
                // Build the retry request from the cloned RequestBuilder,
                // then REPLACE the `Authorization` header on the resulting
                // `Request`. The prior implementation called
                // `b.header("Authorization", new_bearer).send()` —
                // reqwest's `RequestBuilder::header` APPENDS to a
                // HeaderMap (it doesn't replace), so the retry went out
                // with TWO `Authorization: Bearer …` headers (old and
                // new). Graph apparently keyed off the first (old, stale)
                // header and rejected the retry as 401 again —
                // sibling-crates-hunt-2026-06-08 #2 HIGH-severity
                // finding ("auto-refresh silently doesn't work").
                let mut req2 = b.build().context("build retry request")?;
                req2.headers_mut().remove("Authorization");
                req2.headers_mut().insert(
                    "Authorization",
                    HeaderValue::from_str(&self.bearer()).context("encode new bearer for retry")?,
                );
                let resp2 = self.http.execute(req2).context("graph retry")?;
