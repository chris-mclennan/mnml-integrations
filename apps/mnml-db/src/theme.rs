...
//! `~/.config/mnml/current-theme.toml`. This app reads that file
//! and remaps its ratatui named colours onto mnml's `[base_30]`
//! palette, so it matches whatever theme mnml is on — and retints
//! live when mnml switches theme (mtime poll, once per render
//! tick). When the file is absent (mnml not installed / never run)
//! [`remap`] is a no-op, so the app keeps its own colours.
//!
//! Self-contained: no extra deps, no wire-protocol coupling.
...
/// mnml's palette, as ratatui `Color`s.
#[derive(Clone, Copy)]
pub struct Palette {
    pub fg: Color,
    pub dim: Color,
