        ...
        MouseEventKind::Down(MouseButton::Left) => {
            ...
            match clicked_row {
                ...
                Some(crate::app::VisibleRow::Branch { .. })
                    if col_in_body >= 4 && col_in_body <= 5 =>
                {
...
fn draw_tabs(f: &mut Frame, area: Rect, app: &App) {
    let labels: Vec<Line> = app
        ...
        .map(|(i, t)| {
            let suffix = match &t.data {
                ...
                TabData::App(a) => {
                    if a.loading && a.branches.is_empty() {
                        " · loading".to_string()
                    } else if !a.branches.is_empty() {
                        format!(" ({} br)", a.branches.len())
                    } else {
                        String::new()
                    }
                }
    ...
    let tabs = Tabs::new(labels)
        .block(Block::default().borders(Borders::ALL).title(" amplify "))
...
fn draw_apps_table(f: &mut Frame, area: Rect, app: &App) {
    let tab = app.active();
    let TabData::Apps(a) = &tab.data else { return };
    if let Some(err) = &a.last_error {
        let p = Paragraph::new(format!("error: {err}\n\nPress `r` to retry."))
            .style(Style::default().fg(Color::Red));
        f.render_widget(p, area);
    ...
    // Filter or dim per show_hidden mode. `is_hidden_row` is a
    // dim signal, not a filter, when show_hidden is on.
    let rows: Vec<Row> = a
        .items
        .iter()
        .filter(|app_row| a.show_hidden || !hidden.iter().any(|h| h == &app_row.app_id))
        .map(|app_row| {
            let is_hidden_row = hidden.iter().any(|h| h == &app_row.app_id);
            let base = if is_hidden_row {
                Style::default()
                    .fg(Color::DarkGray)
            ...
            let id_style = if is_hidden_row {
                base
            } else {
                Style::default().fg(Color::Yellow)
            };
            let name_style = if is_hidden_row {
                base
            } else {
                Style::default().add_modifier(Modifier::BOLD)
            };
            let repo_style = if is_hidden_row {
                base
            } else {
                Style::default().fg(Color::Cyan)
            };
            let name_text = if is_hidden_row {
                format!("{}  · hidden", app_row.name)
            } else {
                app_row.name.clone()
            };
            Row::new(vec![
                Cell::from(short_id(&app_row.app_id)).style(id_style),
                Cell::from(name_text).style(name_style),
                Cell::from(app_row.platform.clone().unwrap_or_default()).style(base),
                Cell::from(
                    app_row
                        .repository
                        .as_deref()
    ...
    let hint = if a.show_hidden {
        format!(" {} · show-hidden ", tab.name)
    } else {
        format!(" {} ", tab.name)
    };
    let table = Table::new(rows, widths)
...
/// Which branches should always appear in the App-tab list?
/// - Anything with a well-known "primary" name (main, master,
///   develop, staging, production).
/// - Anything under `release/`.
/// Everything else is "auxiliary" (feature/*, bugfix/*, whatever),
/// and only the SINGLE most-recently-deployed auxiliary branch is
/// shown — the rest are collapsed under a "+N more" footer row so
/// the list stays scannable on repos with dozens of open branches.
...
/// Sort key for "most recent deploy" — later timestamp = earlier
/// in sort. Missing timestamps sort last.
fn last_deploy_ts(a: &crate::app::AppTab, branch: &str) -> String {
    a.jobs_by_branch
        .get(branch)
...
fn draw_app_view(f: &mut Frame, area: Rect, tab: &TabState) {
    let TabData::App(a) = &tab.data else { return };
    if let Some(err) = &a.last_error {
        let p = Paragraph::new(format!("error: {err}\n\nPress `r` to retry."))
            .style(Style::default().fg(Color::Red));
        f.render_widget(p, area);
    ...
    // 2026-07-03 UX pass: single full-width list. Primary branches
    // (main / develop / staging / release/*) always show. Auxiliary
    // branches (feature/* etc) collapse to the one most-recently
    // deployed + a "+N more" footer.
    let mut primary_indices: Vec<usize> = Vec::new();
    let mut aux_indices: Vec<usize> = Vec::new();
    for (i, b) in a.branches.iter().enumerate() {
        if is_primary_branch(&b.branch_name) {
            primary_indices.push(i);
        } else {
            aux_indices.push(i);
        }
    }
    // Order primaries by our conventional priority (main first,
    // then develop, staging, release/*, then anything else primary).
    primary_indices.sort_by_key(|&i| {
        let n = a.branches[i].branch_name.as_str();
        match n {
            "main" | "master" => 0,
            "develop" => 1,
            "staging" => 2,
            _ if n.starts_with("release/") => 3,
            _ => 4,
        }
    });
    // Pick the single most-recently-deployed auxiliary branch (if any).
    aux_indices.sort_by(|&a_i, &b_i| {
        last_deploy_ts(a, &a.branches[b_i].branch_name)
            .cmp(&last_deploy_ts(a, &a.branches[a_i].branch_name))
    });
    let (top_aux, collapsed_aux_count): (Option<usize>, usize) = match aux_indices.as_slice() {
        [] => (None, 0),
        [only] => (Some(*only), 0),
        [first, rest @ ..] => (Some(*first), rest.len()),
    };
    let mut visible: Vec<usize> = primary_indices;
    if let Some(i) = top_aux {
        visible.push(i);
    }

    let branches_rows: Vec<Row> = visible
        .iter()
        .map(|&i| {
            let b = &a.branches[i];
            let stage = b.stage.clone().unwrap_or_default();
            let stage_style = match stage.as_str() {
                "PRODUCTION" => Style::default().fg(Color::Green),
                "BETA" => Style::default().fg(Color::Yellow),
                "DEVELOPMENT" => Style::default().fg(Color::Cyan),
            ...
            // Per-branch job history — jobs[0] is newest.
            let jobs = a.jobs_by_branch.get(&b.branch_name);
            let (current_cell, last_cell) = match jobs {
                None => (
                ...
                Some(js) if js.is_empty() => (
                    Cell::from("—").style(Style::default().fg(Color::DarkGray)),
                    Cell::from("(no deploys)").style(Style::default().fg(Color::DarkGray)),
                ),
                Some(js) => {
                    let is_in_flight =
                        |s: &str| matches!(s, "RUNNING" | "PENDING" | "PROVISIONING");
                    let (current_j, last_j) = if is_in_flight(&js[0].status) {
                        (Some(&js[0]), js.get(1))
                    } else {
                        (None, Some(&js[0]))
                    };
                    let current_cell = match current_j {
                        Some(j) => {
                            let text = format!("#{}  {}", j.job_id, j.status);
                            Cell::from(text).style(Style::default().fg(Color::Cyan))
                        }
                        None => Cell::from("—").style(Style::default().fg(Color::DarkGray)),
                    };
                    let last_cell = match last_j {
                        Some(j) => {
                            let color = match j.status.as_str() {
                                "SUCCEED" => Color::Green,
                                "FAILED" | "CANCELLED" => Color::Red,
                            ...
                            let short_when = when
                                ...
                                .map(str::to_string)
                                .unwrap_or_default();
                            let text = format!("#{}  {}  {}", j.job_id, j.status, short_when);
            ...
            Row::new(vec![
                Cell::from(b.branch_name.clone())
                    .style(Style::default().add_modifier(Modifier::BOLD)),
                Cell::from(stage).style(stage_style),
                current_cell,
                last_cell,
            ])
        })
    ...
    let branches_header = Row::new(vec![
        Cell::from("BRANCH"),
        Cell::from("STAGE"),
        Cell::from("CURRENT"),
        Cell::from("LAST DEPLOY"),
    ])
    .style(
        Style::default()
            .fg(Color::DarkGray)
    ...
    let title = if collapsed_aux_count == 0 {
        format!(" branches ({}) ", visible.len())
    } else {
        format!(
            " branches ({} shown · +{} feature branches hidden — press `A` for all) ",
            visible.len(),
            collapsed_aux_count
        )
    };
    let branches_table = Table::new(branches_rows, branches_widths)
        .header(branches_header)
        .block(Block::default().borders(Borders::ALL).title(title))
        .row_highlight_style(
            Style::default()
                .bg(Color::DarkGray)
    ...
    let mut state = TableState::default();
    state.select(Some(a.selected));
    f.render_stateful_widget(branches_table, area, &mut state);
}
