...
//!
//! `glyph_codepoint = "F1B06"` pins the SVG at the same codepoint
...
fn sns_svg_path() -> Option<PathBuf> {
    if let Ok(exe) = std::env::current_exe()
        && let Some(dir) = exe.parent()
    {
        let cand = dir.join("assets/icons/sns.svg");
        if cand.exists() {
            return Some(cand);
        }
        let mut cur = dir.to_path_buf();
        while cur.pop() {
            let cand = cur.join("assets/icons/sns.svg");
            if cand.exists() {
                return Some(cand);
            }
        }
    ...
    let cand = cwd.join("assets/icons/sns.svg");
    if cand.exists() {
        return Some(cand);
    }
    None
...
pub fn install() -> Result<()> {
    let spec = IntegrationSpec {
        ...
        chip: Some(ChipSpec {
            ...
            glyph_svg: sns_svg_path(),
            glyph_codepoint: Some("F1B06".into()),
        }),
