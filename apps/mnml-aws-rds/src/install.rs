...
//!
//! `glyph_codepoint = "F1B04"` pins the SVG at the same codepoint
//! mnml core used to bake it at (see `src/glyph_builder.rs`'s
//! `BUILTIN_GLYPHS` in mnml pre-Stage-2), so users who already have
//! MnmlSymbols.ttf on their system don't see the rds chip change
...
const INTEGRATION_ID: &str = "rds";

fn rds_svg_path() -> Option<PathBuf> {
    if let Ok(exe) = std::env::current_exe()
        && let Some(dir) = exe.parent()
    {
        let cand = dir.join("assets/icons/rds.svg");
        if cand.exists() {
            return Some(cand);
        }
        let mut cur = dir.to_path_buf();
        while cur.pop() {
            let cand = cur.join("assets/icons/rds.svg");
            if cand.exists() {
                return Some(cand);
            }
        }
    ...
    let cand = cwd.join("assets/icons/rds.svg");
    if cand.exists() {
        return Some(cand);
    }
    None
...
pub fn install() -> Result<()> {
    let spec = IntegrationSpec {
        ...
        chip: Some(ChipSpec {
            ...
            glyph_svg: rds_svg_path(),
            glyph_codepoint: Some("F1B04".into()),
        }),
