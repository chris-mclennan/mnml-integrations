...
//! manifest at `~/.config/mnml/integrations/cognito.toml` so mnml
//! picks up the rail chip + palette command + chord binding on
//! next startup.
//!
//! 2026-08-01 — Stage 2 of the mnml-bridge 0.4 sibling-icons SDK.
//! Instead of relying on mnml core baking `assets/glyphs/aws/cognito.svg`
//! into MnmlSymbols.ttf at a codepoint mnml chose, we now ship our
//! own SVG in-repo (`assets/icons/cognito.svg`) and declare it via
//! `ChipSpec::glyph_svg`. `install_integration` copies the SVG to
//! `~/.config/mnml/glyphs/cognito.svg`; mnml discovers it on next
...
pub fn install() -> Result<()> {
    let spec = IntegrationSpec {
        ...
        label: "Cognito User Pools + users".into(),
        description: Some("Cognito User Pool + users with `/` search/filter".into()),
        version: Some(env!("CARGO_PKG_VERSION").into()),
        binary: "mnml-aws-cognito".into(),
